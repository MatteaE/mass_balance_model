...
run_params <- list(
  ...
  filename_points_daily_out    =   "",             # File name of the points where to store daily output
  ...
  deposition_mass_lim          =   2000,                           # [kg m-2]: maximum snow deposition in a cell during an avalanche. A lower value makes avalanches travel farther. Called D_lim in Gruber (2007). Better set it - if left at NA, it is very empirically estimated from the maximum annual solid precipitation.
  ...
  initial_snowline_elevation   =   3700,                         # [m]: initial snow line elevation, at the beginning of each simulated year. If NA, it is estimated automatically from the first DEM grid.
  ...
  initial_snow_avalanche       =   TRUE,                          # [TRUE/FALSE]: if TRUE, the calculation of initial snow distribution also includes the effect of avalanche (which is affected by the parameters in the previous "avalanche" section).
  initial_snow_dist_from_model =   FALSE,                         # [TRUE/FALSE]: if TRUE, the starting condition for the simulation is taken from the respective simulated SWE of the previous year's model run, if available. Then the other "initial_" parameters are ignored. If FALSE, compute initial SWE from topography and given parameters. The first simulated year always uses a computed initial SWE since there is no previous modeled year.
  ...
  default_prec_corr            =   2000,                          # [%]: default precipitation correction in case no winter data or annual parameters are available.
  default_prec_summer_fact     =   c(1,1,1,1,0.4,0.1,0.1,0.1,0.4,0.8,1,1),                       # [-]: default multiplicative reduction of precipitation correction in summer. It can be either 1 number (applied from 1 May to 30 September), or 12 numbers (applied month_wise).
  default_prec_elegrad         =   10,                        # [% / 100 m]: default altitudinal gradient of precipitation. It can be either 1 number (applied every day of the year), or 12 numbers (applied month-wise). 
  default_temp_elegrad         =   -0.65,                      # [°C / 100 m]: default altitudinal gradient of air temperature. It can be either 1 number (applied every day of the year), or 12 numbers (applied month-wise).
  ...
  show_contours                =   TRUE,                        # In the mass balance maps, show the contour lines (TRUE) or not (FALSE).
  ...
  last_year                    =   2017                          # Last modeled year (same as previous comment)
