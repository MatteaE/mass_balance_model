...
run_params <- list(
  ...
  file_weather_nskip                = 1,                          # [-]: number of header lines to skip in the meteo file. The first non-skipped line should already have the first data entry (no header)
  ...
  last_year                         = 1977,                       # [-]: the last year to be processed. If same as first_year, a single year is simulated. Else the model covers multiple years.
  

  # . (1) Input files parameters --------------------------
  filename_massbalance_annual       = "mb_minimal_annual.dat",    # [filename in "quotes"]: file (under massbal/) with the annual mass balance observations
  filename_massbalance_winter       = "mb_minimal_winter.dat",    # [filename in "quotes"]: file (under massbal/) with the winter mass balance observations
  
  
  # . (6) Avalanche model parameters ----------------------
  model_avalanche_dates             = c("1/31", "3/31", "6/30")   # [month/day in "quotes"]: dates at which an avalanche is simulated
