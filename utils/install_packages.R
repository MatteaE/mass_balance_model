...
func_install_all <- function() {
  ...
  # Change the timeout, to allow long downloads on slow connections.
  timeout_old <- options("timeout")
  ...
  still_missing_names <- setdiff(c(packages_cran, packages_github_names, "topmodel", "notifier"), rownames(installed.packages()))
  if (length(still_missing_names) > 0) {
  ...
  # Restore initial value for timeout.
  options("timeout" = timeout_old$timeout)
