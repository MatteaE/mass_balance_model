...
# 0 or "n(ormal)" calls message() with no formatting
...
func_customlog <- function(..., level = c("normal", "warning", "fatal", "success")) {
  ...
  if (is.character(level)) {
    ...
    if (level %in% allowed_levels) {
      level <- c("normal", "warning", "fatal", "success")[level[1]+1]
    } else {
