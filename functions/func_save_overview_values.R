...
func_save_overview_values <- function(year_data,
  ...
  if (year_data$nstakes_annual > 0) {
    ...
    df_overview$mb_annual_meas_corr[year_data$year_id]  <- year_data$massbal_annual_values[["meas_period_corr"]] / 1e3
    df_overview$mb_annual_meas[year_data$year_id]       <- year_data$massbal_annual_values[["meas_period"]] / 1e3
    if (year_data$process_winter) {
      df_overview$mb_winter_meas[year_data$year_id]     <- year_data$massbal_winter_values[["meas_period"]] / 1e3
    }
    df_overview$rmse[year_data$year_id]                 <- year_data$mod_output_annual_cur$global_rms / 1e3
