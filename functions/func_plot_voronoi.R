...
func_plot_voronoi <- function(year_data,
  ...
  theme_map_voronoi <- theme_void(base_size = plots_map_common_elements$base_size) +
    theme(legend.position = "bottom",
          ...
          legend.box.margin = margin(0,0,5*plots_map_common_elements$base_size/16,0),
  ...
  title_cur <- func_make_map_title(
    list(paste0(year_data$year_cur-1, "/", year_data$year_cur),
         ...
         paste0("Area-weighting factor: ", sprintf("%.2f", run_params$optim_annual_areaweight_fact))),
  ...
  if (year_data$process_winter) {
    ...
    title_cur <- func_make_map_title(
      list(paste0(year_data$year_cur-1, "/", year_data$year_cur),
           ...
           paste0("Area-weighting factor: ", sprintf("%.2f", run_params$optim_winter_areaweight_fact))),
      base_size = plots_map_common_elements$base_size)
