...
func_find_input_files_single <- function(run_params,
  ...
  dir_files_interesting_years    <- sort.int(as.integer(str_remove_all(str_remove_all(dir_files_interesting_names, coll(filename_prefix)), coll(filename_suffix))))
  dir_files_interesting_abspaths <- normalizePath(file.path(dirpath, dir_files_interesting))
  
  run_params[[paste0(input_type, "_years")]] <- dir_files_interesting_years
