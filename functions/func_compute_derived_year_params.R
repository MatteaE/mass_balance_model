...
func_compute_derived_year_params <- function(year_data,
                                             year_cur_params,
                                             run_params) {
  ...
  # hydro end to October/April 1 (else we would miss the mass balance
  # between YYYY/09/30 00:00 and YYYY/10/01 00:00).
  # This is why we use hydro_start_mmdd for both hydro_start and hydro_end.
  # These expressions work for both Northern and Southern Hemispheres.
  year_cur_params$hydro_start        <- as.Date(paste0(year_data$year_cur-1, "/", run_params$hydro_start_mmdd), format="%Y/%m/%d")
  year_cur_params$hydro_end          <- as.Date(paste0(year_data$year_cur,  "/", run_params$hydro_start_mmdd), format="%Y/%m/%d")
  year_cur_params$fixed_winter_start <- as.Date(paste0(year_data$year_cur-1, "/", run_params$massbal_fixed_winter_start), format = "%Y/%m/%d")
  
  # This is different because in the Southern Hemisphere
  # the winter period ends in the YYYY-1 calendar year.
  if (run_params$north_south == "North") {
    year_cur_params$fixed_winter_end <- as.Date(paste0(year_data$year_cur, "/", run_params$massbal_fixed_winter_end), format = "%Y/%m/%d")
  } else {
    year_cur_params$fixed_winter_end <- as.Date(paste0(year_data$year_cur-1, "/", run_params$massbal_fixed_winter_end), format = "%Y/%m/%d")
  }
